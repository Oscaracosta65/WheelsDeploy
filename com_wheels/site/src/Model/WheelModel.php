...
class WheelModel extends BaseDatabaseModel
{
     ...
     *  1. Same pickSize AND same assurance (preferring the precomputed assuranceCode
     *     field when available on both wheels, otherwise falling back to assuranceText)
    ...
    public function getSimilarWheels(array $wheel, int $limit = 5): array
    {
        ...
        $assuranceCode   = (string) ($wheel['assuranceCode'] ?? '');
        ...
        foreach ($wheels as $id => $candidate) {
            ...
            // Prefer assuranceCode for matching when both wheels have it.
            $candCode = (string) ($candidate['assuranceCode'] ?? '');
            $candText = (string) ($candidate['assuranceText'] ?? '');

            if ($assuranceCode !== '' && $candCode !== '') {
                if ($candCode !== $assuranceCode) {
                    continue;
                }
            } else {
                if ($candText !== $assuranceText) {
                    continue;
                }
            }
     ...
     *
     * Prefers the precomputed "categoryKeys" array when present; falls back to
     * deriving slugs from pickSize, requiredSelections, and assuranceText.
     *
    ...
    public function getWheelCategories(array $wheel): array
    {
        ...
        if (isset($wheel['categoryKeys']) && is_array($wheel['categoryKeys'])) {
            ...
            foreach ($wheel['categoryKeys'] as $slug) {
                $slug          = (string) $slug;
                $result[$slug] = CategoryHelper::getLabelFromSlug($slug);
            }

            return $result;
        }

        // Fall back to computed categories
        $pickSize           = (int) ($wheel['pickSize'] ?? 0);
